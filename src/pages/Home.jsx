...
function Home() {
  ...
  const fixtures = Array.isArray(appData.fixtures) ? appData.fixtures : [];
  const results = Array.isArray(appData.results) ? appData.results : [];
  const news = Array.isArray(appData.news) ? appData.news : [];
  const sports = Array.isArray(appData.sports) ? appData.sports : [];
  const teams = Array.isArray(appData.teams) ? appData.teams : [];
  const favourites = Array.isArray(appData.favourites) ? appData.favourites : [];
  const getSortedTable =
    typeof appData.getSortedTable === "function"
      ? appData.getSortedTable
      : () => [];
  ...
  const upcomingFixtures = fixtures
    .filter((fixture) => fixture?.status === "Upcoming" && !fixture?.postponed)
    .slice(0, 2);
  ...
  const qualifiedTeams = facultyDepartments.length;
    ...
    return teams.filter((team) => {
      ...
      const categoryMatch =
        searchCategory === "all" || category === searchCategory.toLowerCase();
  ...
  const handleSearch = () => {
    ...
    const matchedTeam = searchableTeams.find((team) =>
    ...
    const matchedPlayer = searchableTeams
      .flatMap((team) =>
      ...
      .find((player) => String(player?.name || "").toLowerCase().includes(query));
                  ...
                  sports.map((sport) => (
                    ...
                    >
                      <div className="homepage-sport-card__image-wrap">
                        <img
                          src={
                            String(sport?.name || "").toLowerCase() === "football"
                              ? "https://images.unsplash.com/photo-1574629810360-7efbbe195018?auto=format&fit=crop&w=900&q=80"
                              : String(sport?.name || "").toLowerCase() === "basketball"
                              ? "https://images.unsplash.com/photo-1546519638-68e109498ffc?auto=format&fit=crop&w=900&q=80"
                              : String(sport?.name || "").toLowerCase() === "volleyball"
                  ...
                  <div className="homepage-side-list">
                    {recentResults.map((result) => (
                      <div className="homepage-side-card" key={result?.id}>
                        ...
                        <p>
                          {result?.sport || "Sport"} • {result?.gender || result?.category || "Category"}
                        </p>
                      </div>
                    ))}
                  </div>
